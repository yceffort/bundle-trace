...
- `scripts/verify-package.mjs`: packs `@yceffort/coldpath` and a platform analyzer package, installs both into a fresh npm project, and runs the Rollup plugin, `coldpath graph`, `coldpath collect --scenarios`, and `coldpath analyze --scenarios` there.
