...
export async function measuredDetails(root, artifacts, binary = fileURLToPath(new URL('../target/release/coldpath', import.meta.url))) {
  const directory = await mkdtemp(resolve(tmpdir(), 'coldpath-intervals-'))
  try {
    const paths = [...new Set(artifacts.flatMap((artifact) => artifact.scripts.map((script) => script.path)))]
...
export async function verifyReport(root, artifacts, report, binary) {
  ...
  const detailedBundles = new Map(details?.bundles.map((bundle) => [bundle.path, bundle]) || [])
  let checked = 0
