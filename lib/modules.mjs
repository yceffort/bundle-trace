...
function tableEntries(table) {
  if (table?.type === 'ObjectExpression') {
    return table.properties.filter((p) => ['ObjectProperty', 'ObjectMethod'].includes(p.type) && !p.computed)
      .map((p) => [String(p.key.value ?? p.key.name), p.type === 'ObjectProperty' ? p.value : p, p])
  }
  if (table?.type === 'ArrayExpression') return table.elements.map((e, i) => e && [String(i), e, e]).filter(Boolean)
  return []
}
// `start`/`end`: the recovered source (the factory body). `header`: where the entry, id included, begins.
const tableModules = (entries) => entries.map(([id, factory, node]) => isFunction(factory)
  ? {id, header: node.start, start: factory.body.start, end: factory.end, factory}
  : {id, header: node.start, start: node.start, end: node.end})
const isModuleTable = (entries) => entries.length > 0 && entries.every(([, factory]) => isFunction(factory))
...
function runtimeModules(program) {
  for (const statement of program.body) {
    ...
    const tables = [[fn.params[0], call.arguments[0]], ...fn.body.body.filter((s) => s.type === 'VariableDeclaration')
      .flatMap((s) => s.declarations.map((d) => [d.id, d.init]))]
    let names = null, global = null
    const scan = (node) => {
      ...
      if (node.type === 'AssignmentExpression' && node.left.type === 'MemberExpression' && node.right.type === 'LogicalExpression' &&
        node.right.right.type === 'ArrayExpression' && !node.right.right.elements.length && !node.left.computed) global ??= node.left.property.name
