...
function tableEntries(table) {
  if (table?.type === 'ObjectExpression') {
    return table.properties
      .filter((p) => ['ObjectProperty', 'ObjectMethod'].includes(p.type) && !p.computed)
...
function runtimeModules(program) {
  for (const statement of program.body) {
    ...
    const tables = [
      ...
      ...fn.body.body.filter((s) => s.type === 'VariableDeclaration').flatMap((s) => s.declarations.map((d) => [d.id, d.init])),
    ]
    let names = null,
      global = null
    const scan = (node) => {
...
export function chunkModules(code) {
  ...
  const calls = ast.program.body.flatMap(({expression: e}) => (e?.type === 'SequenceExpression' ? e.expressions : [e]))
...
function bindingNames(pattern, names) {
  ...
  if (pattern.type === 'Identifier') names.add(pattern.name)
    ...
    pattern.properties.forEach((p) => bindingNames(p.type === 'RestElement' ? p.argument : p.value, names))
