// Recover module boundaries from map-less webpack chunks as synthetic source maps.
...
// `(self.webpackChunkX = self.webpackChunkX || []).push([[chunkIds], {id: factory} | [factory]])`
export function webpackModules(code) {
  let ast
  try {
  ...
  for (const statement of ast.program.body) {
    ...
    const target = call.callee.object.type === 'AssignmentExpression' ? call.callee.object.left : call.callee.object
    const global = target.type === 'MemberExpression' ? target.property.name : target.name
    const modules = call.arguments[0]?.elements?.[1]
...
export async function inferModules({dir, out}) {
  ...
  for (const file of await scripts(dir)) {
    ...
    const found = webpackModules(code)
  ...
  console.log(`Recovered ${modules} webpack modules in ${Object.keys(bindings).length} chunks -> ${join(out, 'maps.json')}`)
  return bindings
}
