...
function valid(value, schema) {
  ...
  return schema.required.every((key) => {
    const rule = schema.properties[key], field = value[key]
    return rule.type === 'array' ? Array.isArray(field) && field.every((item) => typeof item === 'string') :
      typeof field === 'string' && (!rule.enum || rule.enum.includes(field))
  })
}
...
function digest(code) {
  ...
  return {
    ...
    strings: unique([...code.matchAll(/(["'`])((?:(?!\1)[^\\\n]){4,80})\1/g)].map((m) => m[2]).filter((s) => /[A-Za-z]{3}/.test(s)), 80),
    keys: unique([...code.matchAll(/[{,]([A-Za-z_$][\w$]{3,40}):/g)].map((m) => m[1]), 60),
  }
}
...
export async function label({report, out, top = 50, mode = 'identify', provider = 'anthropic', model, baseUrl, lang = 'English', concurrency = 5}) {
  ...
  const schema = mode === 'identify' ? identifySchema(lang) : describeSchema(lang)
  ...
  async function ask(content) {
    ...
    if (client) {
      const response = await client.messages.create({
        model, max_tokens: 1024, system: SYSTEM[mode], messages: [{role: 'user', content}],
      ...
      const response = await fetch(`${baseUrl ?? 'https://api.openai.com/v1'}/chat/completions`, {
        ...
        body: JSON.stringify({model, messages: [{role: 'system', content: SYSTEM[mode]}, {role: 'user', content}],
  ...
  await Promise.all(Array.from({length: concurrency}, async () => {
    for (let row; (row = queue.shift());) {
      ...
      const answer = await ask(JSON.stringify({source: row.source, bytes: row.bytes, ...digest(code)}))
      if (!answer) {
        failed++
      ...
      const evidence = answer.evidence.filter((text) => text.length >= 6 && code.includes(text) && docFreq(text) <= maxDocFreq)
