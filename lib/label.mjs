...
export async function label({
  ...
  const inSource = (copies, text) => copies.some((code) => code.includes(text))
  const all = [...contents.values()]
  const maxDocFreq = Math.max(3, Math.ceil(all.length * 0.005))
  const docFreq = (text) => all.reduce((count, copies) => count + inSource(copies, text), 0)
  ...
  await Promise.all(
    Array.from({length: concurrency}, async () => {
      for (let row; (row = queue.shift());) {
        ...
        const distinctive = (list) => list.filter((text) => text.length >= 6 && inSource(copies, text) && docFreq(text) <= maxDocFreq)
        if (task === 'chunk') {
          const contents = answer.contents
            .filter((part) => part.kind !== 'unknown')
            .map((part) => ({...part, evidence: distinctive(part.evidence)}))
            .filter((part) => part.evidence.length)
          if (!contents.length) {
            rejected++
            sources[row.source] = {summary: answer.summary}
