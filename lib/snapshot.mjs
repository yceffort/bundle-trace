...
export async function snapshot({url, out, waitMs = 5000, actions, scenario}) {
  ...
  const entries = [], documents = new Map(), requests = new Map()
  let snapshots = 0, initial
  try {
    ...
    page.on('response', (response) => {
      if (response.request().isNavigationRequest() && response.frame() === page.mainFrame()) {
        documents.set(response.url(), response.text().catch(() => undefined))
      }
    })
    ...
    const sources = new Map(), failures = []
    const record = async () => {
    ...
    cdp.on('Debugger.paused', ({reason, data}) => {
      ...
      snapshot().catch((error) => failures.push(error)).finally(() => cdp.send('Debugger.resume').catch(() => {}))
    })
  ...
  const coverage = [], seen = new Set(), inHtml = new Map()
  const loadCause = async (key, name, documentURL) => {
    ...
    if (!inHtml.has(documentURL)) {
      inHtml.set(documentURL, new Set([...html.matchAll(/<(?:script\b[^>]*\ssrc|link\b[^>]*\shref)\s*=\s*["']([^"']+)["']/gi)]
        .map((m) => new URL(m[1].replaceAll('&amp;', '&'), documentURL).href.split(/[?#]/)[0])))
    }
  ...
  console.log(`${seen.size} scripts (${Object.entries(counts).map(([k, v]) => `${v} ${k}`).join(', ')}), ${snapshots} snapshots, ` +
