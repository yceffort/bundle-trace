...
export async function collect({url, dir, out, prefix = '/', scenario = 'initial', actions, waitMs = 1000, cdnPrefixes = [],
  ...
  const remotes = cdnPrefixes.map((value) => {
    ...
    assert(['http:', 'https:'].includes(remote.protocol) && value.endsWith('/') && !remote.search && !remote.hash,
