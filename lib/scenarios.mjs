...
export async function loadScenarios(file) {
  ...
  const scenarios = config.scenarios.map((scenario) => {
    ...
    const option = (key) => scenario[key] ?? config[key]
    const storageState = option('storageState')
    return {
      device: option('device'), viewport: option('viewport'), userAgent: option('userAgent'),
      deviceScaleFactor: option('deviceScaleFactor'), isMobile: option('isMobile'), hasTouch: option('hasTouch'),
      network: option('network'), cpuSlowdown: option('cpuSlowdown'), cdnPrefixes: option('cdnPrefixes'),
