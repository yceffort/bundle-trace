...
export const sourcePath = (path, root) =>
    ...
    if (mod && !mod.source.startsWith('[') && !mod.source.includes('\0')) {
      ...
      try {
        ...
        if (error.code !== 'ENOENT' && error.code !== 'EISDIR')
          graph.warnings.push(`No parsed import locations for ${mod.source}: ${error.message}`)
      }
    }
...
export function turbopackGraph(bytes, root, environment = 'client') {
  ...
  const modules = header.modules.flatMap((mod, i) =>
