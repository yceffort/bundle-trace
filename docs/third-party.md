...
coldpath modules --dir artifacts/site/files --out artifacts/site/modules
...
- `maps.json`: explicit map bindings for scripts that declare a `sourceMappingURL`. A map the snapshot could fetch is saved under `maps/`. A declared map that could not be fetched is bound to an empty map, so the analyzer treats the script as unmapped instead of failing. The analyzer itself never fetches.
...
`coldpath label --report report.json --out labels.json` reads a report generated with `--details` and asks a language model about the sources with the most unobserved bytes (`--top`, default 50). The model sees a digest of each source: its first 600 characters, up to 80 string literals, and up to 60 property keys. **This sends code to the model provider.** Do not use it on code you may not share.

- `--mode identify` (default) handles only recovered `webpack://inferred/` modules. It asks for a `summary` of what the code does, plus a guessed `name`, `shortName`, `kind` (`package`, `app`, `polyfill`, `data`, or `unknown`), `reasoning`, and `evidence`.
...
The treemap groups bundles by load cause when `--loading` is given, shows labeled sources as `≈ shortName (file)`, includes label names and summaries in search, and shows the summary, guessed identity, reasoning, and evidence on a source's page, marked as a model's guess.
